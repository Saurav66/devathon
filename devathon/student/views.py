...
def userlist(request):
    tok_list = Token.objects.filter(reg_id__user__username=request.user.username)
    print(tok_list)
    flag = False
...
def sendEmail(to, otp):
    try:
        ...
        server.sendmail('singh_821916@student.nitw.ac.in', to, f'Thanks for odering with us. Your OTP is {otp}.')
        server.close()
        return True
